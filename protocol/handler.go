...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NewBlockHashesMsg:
		log.Info("----------received NewBlockHashesMsg ")
...
func (pm *ProtocolManager) BroadcastBlock(block *types.Block, propagate bool) {
	...
	log.Info("-----------propagate:",strconv.FormatBool(propagate)," pid :",os.Getpid())
	if propagate {
	...
	log.Info("------------hasblock:",strconv.FormatBool(pm.blockchain.HasBlock(hash, block.NumberU64()))," pid :",os.Getpid())
	if pm.blockchain.HasBlock(hash, block.NumberU64()) {
		...
		for _, peer := range peers {
			if peer.LocalType() == p2p.NtAccess {
				peer.SendNewBlockHashes([]common.Hash{hash}, []uint64{block.NumberU64()})
				log.Info("---------access---Announced-----------send block to light")
			}
		}
		for _, peer := range peers {
